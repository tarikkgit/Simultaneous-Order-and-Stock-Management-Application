...
public class CustomerDAO {
    ...
    public void addFunction(CustomerDTO customerDTO) {
        try {
            String query = "INSERT INTO customers (customercode, fullname, location, phone, username, password, balance) VALUES (?, ?, ?, ?, ?, ?, ?)";
            ...
            prepStatement.setString(5, customerDTO.getUsername());
            prepStatement.setString(6, customerDTO.getPassword());
            
            // Rastgele bir bakiye atama
            Random random = new Random();
            double balance = 500 + (random.nextDouble() * (3000 - 500));
            prepStatement.setDouble(7, balance); // Rastgele bakiyeyi ekle
    ...
    public void editCustomerDAO(CustomerDTO customerDTO) {
        try {
            String query = "UPDATE customers SET fullname=?, location=?, phone=?, username=?, password=? WHERE customercode=?";
            prepStatement = conn.prepareStatement(query);
            ...
            prepStatement.setString(4, customerDTO.getUsername());
            prepStatement.setString(5, customerDTO.getPassword());
            prepStatement.setString(6, customerDTO.getCustCode());
            prepStatement.executeUpdate();
    ...
    public ResultSet getCustomerData(int customerId) {
        ResultSet resultSet = null;
        try {
            String query = "SELECT cid, customerType, fullname, location,balance, phone, username, password FROM customers WHERE cid = ?";
            prepStatement = conn.prepareStatement(query);
    ...
    public ResultSet getQueryResult() {
        try {
            String query = "SELECT customercode, fullname, location, phone, username, password FROM customers";
            resultSet = statement.executeQuery(query);
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public ResultSet validateCustomerLogin(String username, String password) {
        try {
            String query = "SELECT cid FROM customers WHERE username=? AND password=?";
            prepStatement = conn.prepareStatement(query);
